...
# This way the script will get latest tag for the each device
# And since the 7th gen devices are on bottom of the array, tag used for the release will be the latest one
function get_tag() {
...
curl https://raw.githubusercontent.com/maade69/adevtool/13/scripts/append-sha256.py --output vendor/adevtool/scripts/append-sha256.py > /dev/null
chmod +x vendor/adevtool/scripts/append-sha256.py
source script/envsetup.sh &> /dev/null
rm -rf ${source_dir}/vendor/adevtool/dl/*
yarn install --cwd vendor/adevtool &> /dev/null
